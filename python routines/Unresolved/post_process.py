#load segmentation and dict, add CN number etc accordingly.
#how to merge? - look at all surviving sads, compute ratios, if ratio>thresh then execute merge on statistics and on segmentation.
# for segmentation merge change all ids of 1st to second. to do it in one go lets have a list of changed ids, with a dict mapping the change
# for stats use the dict to execute all changes. add to second from first
# when writing extremum graph write max ids too, and write this ratio too so you can visually just check once to decide on thresh
...
for key in max_data_dict.keys():
    if(key in remove_max):
        new_max=max_removal_dict[key]
#        print(max_data_dict[new_max])
        max_data_dict[new_max][3]+=max_data_dict[key][3]
        max_data_dict[new_max][4]=dia_from_vol(max_data_dict[new_max][3])
        max_data_dict[new_max][5]+=max_data_dict[key][5]-1
#        print(max_data_dict[new_max])

merged_seg=[]

for point in particle_segmentation:
    new_point=[]
    if(point[3] in remove_max):
        point[3]=max_removal_dict[point[3]]
    new_point.extend(point)
    new_point.append(max_data_dict[point[3]][3])
    new_point.append(max_data_dict[point[3]][4])
    new_point.append(max_data_dict[point[3]][5])
    merged_seg.append(new_point)
